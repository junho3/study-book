...
시스템의 가치를 높이기 위해 반드시 시스템의 동작을 변경할 필요는 없다는 의미입니다.  
이는 시스템의 기능이나 성능을 변화시키지 않더라도, 시스템 자체의 관리나 운영 방식 등을 통해 가치를 높일 수 있다는 뜻입니다.  

다음에 무엇을 할 수 있는지에 대해 선택할 수 있는 기회를 만들자마자, 저는 이미 돈을 번 것입니다.

사용자가 다음에 무엇을 할 수 있는지에 대한 선택의 기회를 제공함으로써, 이미 경제적 가치를 창출할 수 있다는 의미입니다.   
즉, 사용자가 다양한 선택을 할 수 있도록 시스템을 설계하거나 운영하면, 이는 곧 수익으로 이어질 수 있다는 것을 강조합니다.  
