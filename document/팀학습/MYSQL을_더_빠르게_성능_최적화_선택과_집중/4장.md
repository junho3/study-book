...
복제본은 고가용성에 기반을 두므로 소스와 같은 가용성(99.95%, 99.99%)을 가져야 합니다.  

#### 유연성

고성능 하드웨어를 사용하지 않는 순수 읽기 복제본이 있을 때 사소한 읽기에 리소스를 낭비하지 마세요.  
대용량 스토리지, 작은 CPU와 메모리로 읽기 복제본을 제공하는 것은 클라우드 환경에서 더 일반적입니다.  
...
애플리케이션은 캐시 갱신, 무효화, 제거를 관리해야 합니다.  
애플리케이션에는 일반적으로 키-값 모델인 캐시와 함께 작동하는 데이터 모델이 필요합니다.  

캐싱은 자주 접근하지만 드물게 변경되는 데이터에 적합합니다.  

> 캐시 히트율

캐시를 사용할 때 주의할 점은 캐시가 일시적인지 또는 지속 가능한지 결정하는 것입니다.  
캐시가 정말 일시적이라면 애플리케이션에 영향을 주지 않고 캐시 데이터에 대해 `TRUNCATE TABLE`과 같은 작업을 수행할 수 있어야 합니다.  
또한 임시 캐시를 재구성하는 방법도 결정해야 합니다.  
일부 애플리케이션은 캐시가 누락되었을 때 캐시를 다시 작성합니다.  


### 쓰기 대기열에 넣기

대기열(queue)을 사용하여 쓰기 처리량을 안정화합니다.  

대기열을 사용하면 엄청난 양의 요청들이 MySQL에 영향을 주지 않습니다.  
유일한 차이점은 일부 변경 사항이 평소보다 늦게 처리된다는 것입니다.  
MySQL 엔터프라이즈 에디션, Percona 서버, MariaDB 서버에는 동시에 실행되는 쿼리 수를 제한하는 스레드 풀이 있어 스로틀 역할을 합니다.  

운이 매우 좋다면 MySQL이 엄청난 양의 요청들을 처리하거나 아니면 그렇지 못할 것입니다.  
운에 의존하지 마세요.  

쓰기 대기열에 넣기는 MySQL 가용성에서 애플리케이션을 분리한다는 것입니다.  
또 다른 장점은 손실되거나 버려진 변경 사항을 복구하는 데 사용할 수 있다는 것입니다.  
세 번째 장점은 대기열이 Kafka와 같은 이벤트 스트림인 경우 변경 사항을 재실행할 수 있다는 것입니다.  

> 쓰기 대기열에 MySQL(RDBMS) 자체적인 기능이라기보다 Kafka 등을 적용한 아키텍처를 의미하는 것인지?


### 데이터 분할

데이터를 삭제허가나 보관할 수 없을 때 최소한 데이터를 분할해야 합니다.  
MySQL은 파티셔닝을 지원하지만 특별한 처리가 필요합니다.  
따라서 MySQL 파티셔닝을 사용ㅇ하지 않는 것이 좋습니다.  

애플리케이션 개발자가 구현하기에 가장 유용하고 일반적이며 더 쉬운 데이터 파티셔닝 유형은 핫 데이터와 콜드 데이터를 분리하는 것입니다.  

콜드 데이터를 다른 곳에 저장하면 3가지 즉각적인 이점이 있습니다.  
- 더 많은 핫 데이터가 메모리에 적합하고, 쿼리가 콜드 데이터를 검사하는 데 시간을 낭비하지 않음
- 스키마 변경 작업이 더 빠름
- 

통제된 `INSERT ...SELECT`문을 사용하여 핫 테이블에서 선택하고 콜드 테이블에 삽입하면 비교적 쉽습니다.  
...
MySQL은 최선의 선택이 아닐 때에도 광범위한 데이터와 접근 패턴에 대해 놀라운 확장성을 제공합니다.  
그러나 이것을 당연하게 여기지 마세요.  
팀에서 "MySQL이 최선의 선택이 아닐 수도 있어요."라고 말하는 최초의 엔지니어가 되어 보세요.  


## 4-6 더 좋고 빠른 하드웨어?

그러나 때때로 엔지니어는 그러한 애플리케이션을 변경하는 데 필요한 여유 시간이 없습니다.  

왼쪽 열에서 모든 항목과 오른쪽 열에서 최소 2개 이상의 항목에 체크해야 한다면 강력하게 하드웨어를 확장해야 할 시점이 되었음을 의미합니다.  

그러나 하드웨어의 한 부분에서 과부하가 걸리면 일반적으로 다른 하드웨어에 영향을 미치기 시작합니다.  
자체 하드웨어를 실행할 때 최대 스토리지 IOPS는 저장 장치에 의해 결정됩니다.  

### 읽기 집약(Read-heavy)

읽기 집약적인 접근 패턴에서 지속적으로 IOPS가 높은 것은 부족한 IOPS가 아니라 메모리 부족 때문일 수 있습니다.  
